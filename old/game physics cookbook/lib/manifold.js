...
findCollisionFeaturesSphereSphere = (a, b) => {
  ...
  var contact = add(a.position, scale(d, result.dtp));
...
findCollisionFeaturesSphereOBB = (a, b) => { // a: obb, b: sphere
  //console.log(a, b);
  var result = collisionManifold();
  var closestPoint = closestPointOBB(a, b.position); // (obb, point)
...
plane = (normal, distance) => ({normal, distance});
...
getPlanes = obb => {
  ...
  for(var a of [0, 1, 2]){
    for(var b of [-1, 1]){
      //console.log(scale(A[a], b))
      //console.log(scale(A[a], E[a]));
      result.push(
        plane(scale(A[a], b), b * dot(A[a], add(C, scale(A[a], E[a] * b))))
      );
    }
  }
  return result;
  ...
  for(var i = 0; i < planes.length; i++){
    for(var j = 0; j < edges.length; j++){
      if(clipToPlane(planes[i], edges[j], intersection)){
        //console.log(1, intersection, obb);
        if(pointInOBB(obb, intersection)){
          //console.log(2);
...
findCollisionFeaturesOBBOBB = (A, B) => {
  ...
  var axis = norm(hitNormal);
  
  // Intersection points
  var c1 = clipEdgesToOBB(getEdges(B), A);
  var c2 = clipEdgesToOBB(getEdges(A), B);
  
  //console.log(c1, c2, result.contacts);
  
  for(i of c1) result.contacts.push(i.value);
  for(i of c2) result.contacts.push(i.value);
  ...
  for(i = result.contacts.length - 1; i >= 0; i--){
    ...
    // Remove duplicates
    for(j = result.contacts.length - 1; j > i; j--){
